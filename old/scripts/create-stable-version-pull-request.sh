...
#
# Creates a GitHub Pull Request for making new Stable Release based on the "release/*" branch with the merge into the
# 'master' branch.
#
# Usage:
#   $ sh ./scripts/scripts/create-stable-version-pull-request.sh (<new-release-version-or-semver>)
#
# The <new-release-version-or-semver> argument is optional. By default, script will use the "minor" as a information
# to which version we would like to bump the package.
#

DEFAULT_SEMVER_IDENTIFIER=minor
...
NEW_RELEASE_VERSION=$(./node_modules/.bin/semver $CURRENT_VERSION -i $SEMVER_IDENTIFIER)
...
sh -e ./scripts/create-release-pull-request.sh $TARGET_RELEASE_BRANCH $NEW_RELEASE_VERSION
